...
void llama3_forward(LLama3 *model, const int* inputs, size_t B, size_t T) {
    ...
    for (int l = 0; l < L; l++) {
        ...
        // The input l_ln1 now holds the (already layernormed) input
            ...
            rope_forward(l_qkvr, l_qkvr, model->freqs_cis, B, T, n_head, n_kv_head, n_kv_head, hd, main_stream);
            // 4) attention: att <- softmax(qk^T)v
            float* l_att = (float*)acts.att + l * B * NH * T; // cuDNN needs a smaller FP32 tensor
            attention_forward_cudnn(l_atty, (float*)l_att, l_qkvr, B, T, NH, n_kv_head, n_kv_head, hd, main_stream);
        #else
...
void llama3_backward_and_reduce(LLama3 *model, int* inputs, const int* targets, int grad_accum_steps, int micro_step) {
    ...
    for (int l = L-1; l >= 0; l--) {
        ...
        #ifdef ENABLE_CUDNN
        float* l_att = (float*)acts.att + l * B * NH * T; // cuDNN needs a smaller FP32 tensor
        attention_backward_cudnn(dl_bt4c2, dl_btc, l_qkvr, l_atty, l_att, B, T, NH, n_kv_head, n_kv_head, hd, main_stream);
        #else
        floatX* l_att = acts.att + l * B * NH * T * T;
        ...
        rope_backward_inplace(dl_bt4c2, dl_bt4c2, model->freqs_cis, B, T, NH, n_kv_head, n_kv_head, hd, main_stream);
