...
using cache_type_fwd = std::map<std::tuple<int,int,int,int,int,int, int>, std::shared_ptr<fe::graph::Graph>>;
using cache_type_bwd = std::map<std::tuple<int,int,int,int,int,int>, std::shared_ptr<fe::graph::Graph>>;

// Loosely based on cuDNN frontend samples functions and massively simplified
auto lookup_cache_or_build_graph_fwd(int B, int Hq, int Hk, int Hv, int T, int HS, int is_inference_only) {

    static cache_type_fwd user_maintained_cache_fwd;

    auto key = std::make_tuple(B, Hq, Hk, Hv, T, HS, is_inference_only);
...
auto lookup_cache_or_build_graph_bwd(int B, int Hq, int Hk, int Hv, int T, int HS) {
    static cache_type_bwd user_maintained_cache_bwd;

    auto key = std::make_tuple(B, Hq, Hk, Hv, T, HS);
    ...
    dK->set_output(true).set_dim({B, Hk, T, HS}).set_stride({H * HS * T, HS, H * HS, 1}).set_uid(dK_UID);
    dV->set_output(true).set_dim({B, Hv, T, HS}).set_stride({H * HS * T, HS, H * HS, 1}).set_uid(dV_UID);
...
void attention_forward_cudnn(floatX* out,  // output: (B, T, Hq, HS)
                             ...
                             int B, int T, int Hq, int Hk, int Hv, int HS, cudaStream_t stream) {
    NVTX_RANGE_FN();
    ...
    auto graph = lookup_cache_or_build_graph_fwd(B, Hq, Hk, Hv, T, HS, is_inference_only);

    // Prepare all the tensor pointers for executing the graph
    void* devPtrQ = inp;
    void* devPtrK = (inp + Hq * HS);
    void* devPtrV = (inp + (Hq + Hk) * HS);
...
void attention_backward_cudnn(floatX* dqkvr,                                       // output
    ...
    // Get graph and tensors from cache (or generate it on first use)
    auto graph = lookup_cache_or_build_graph_bwd(B, Hq, Hk, Hv, T, HS);

    // Prepare all the tensor pointers for executing the graph
    void* devPtrQ = qkvr;
    void* devPtrK = (qkvr + Hq * HS);
    void* devPtrV = (qkvr + (Hq + Hk) * HS);
    void* devPtrO = o;
    void* devPtrdO = dout;
