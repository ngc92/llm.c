...
__global__ void rope_backward_kernel1(floatX *dinp, const floatX *dout, const floatX *freqs_cis, int B, int T, int Nq, int Nk, int Nv, int head_dim) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int head_dim_half = head_dim / 2;
    int N = Nq + Nk + Nv;
    if (idx >= B * T * N * head_dim_half) return;
...
void rope_backward_inplace(floatX *dinp, const floatX *dout, const floatX *freqs_cis, int B, int T, int Nq, int Nk, int Nv, int head_dim, cudaStream_t stream) {
    // backward pass of forward, mirrors the forward kernel in setup and indexing
    const int block_size = 128;
    int total_threads = B * T * (Nq + Nk + Nv) * head_dim / 2;
    int num_blocks = CEIL_DIV(total_threads, block_size);
    rope_backward_kernel1<<<num_blocks, block_size, 0, stream>>>(dinp, dout, freqs_cis, B, T, Nq, Nk, Nv, head_dim);
    cudaCheck(cudaGetLastError());
}
