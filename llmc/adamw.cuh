...
__device__ void adamw_update(Tp* params_memory, float* master_params_memory, Tg* grads_memory, Tm* m_memory, float* v_memory, size_t num_parameters,
    ...
    m = lerp(grad, m, beta1);
    stochastic_rounding(m, m_memory+idx, seed + 42);
...
template <typename Tp, typename Tg, typename Tm>
__global__ void adamw_kernel3(Tp* params_memory, float* master_params_memory, Tg* grads_memory, Tm* m_memory, float* v_memory, size_t num_parameters,
...
void adamw_update(Tp* params_memory, float* master_params_memory, Tg* grads_memory, GenericBufferView m_memory, float* v_memory, size_t num_parameters,
    ...
    if(m_memory.dtype() == DType::BF16) {
        adamw_kernel3<<<dim3(num_blocks, num_slices), block_size, 0, stream>>>(
                ...
                get_as<nv_bfloat16>(m_memory), v_memory, num_parameters,
                w_stride, g_stride, s_stride,
        ...
        adamw_kernel3<<<dim3(num_blocks, num_slices), block_size, 0, stream>>>(
                params_memory, master_params_memory, grads_memory,
                get_as<float>(m_memory), v_memory, num_parameters,
                w_stride, g_stride, s_stride,
                learning_rate, beta1, beta2,
                beta1_correction, beta2_correction, eps,
                weight_decay,  grad_scale, seed);
