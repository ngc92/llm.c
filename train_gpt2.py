...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: "GPTConfig"):
        ...
        # if we want banded attention, use a shifted triangular to set additional elements to zero
        if config.attn_window > 0:
            mask -= torch.tril(torch.ones(config.block_size, config.block_size), diagonal=-config.attn_window)
        self.banded = config.attn_window > 0
    ...
    def forward(self, x):
        ...
        if FLASH:
            # flashattention
            if self.banded:
                y = F.scaled_dot_product_attention(q, k, v, attn_mask=self.bias != 0)
            else:
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type: str):
        ...
        assert model_type in {'gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl', 'gpt3-dummy'}
        ...
        config_args = {
            ...
            'gpt3-dummy':   dict(n_layer=12, n_head=12, n_embd=768),  # 124M params
        }[model_type]
        config_args['vocab_size'] = 50257 # always 50257 for GPT model checkpoints
        config_args['block_size'] = 1024 # always 1024 for GPT model checkpoints
        # create a from-scratch initialized minGPT model
        config = GPTConfig(**config_args)
        if model_type == 'gpt3-dummy':
            config.attn_window = 128
        model = GPT(config)
        sd = model.state_dict()
        sd_keys = sd.keys()
